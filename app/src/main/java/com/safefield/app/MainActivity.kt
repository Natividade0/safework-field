...
class MainActivity : Activity() {
    ...
    private fun buildShell(){
        ...
        title.addView(tv("SafeField",30f,amber,true))
        title.addView(tv("Segurança do Trabalho em campo",13f,muted,false))
    ...
    private fun tv(s:String,sz:Float,c:Int,b:Boolean)=TextView(this).apply{text=s;textSize=sz;setTextColor(c);if(b)setTypeface(Typeface.DEFAULT_BOLD)}
    private fun card()=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;background=bg(cardColor,18,Color.rgb(39,45,57));setPadding(dp(16),dp(14),dp(16),dp(16))}
    private fun addCard(v:LinearLayout){content.addView(v,LinearLayout.LayoutParams(-1,-2).apply{setMargins(0,0,0,dp(12))})}
    private fun btn(s:String,primary:Boolean,on:()->Unit)=Button(this).apply{text=s;textSize=14f;setTextColor(if(primary)Color.BLACK else textColor);background=bg(if(primary)amber else Color.rgb(28,33,43),16,if(primary)null else Color.rgb(49,57,72));setOnClickListener{on()}}
    private fun section(p:LinearLayout,s:String){p.addView(tv(s,17f,amber,true).apply{setPadding(0,dp(10),0,dp(8))})}
    private fun input(p:LinearLayout,k:String,h:String,multi:Boolean=false):EditText{val e=EditText(this).apply{hint=h;setTextColor(textColor);setHintTextColor(Color.rgb(100,110,125));setSingleLine(!multi);minLines=if(multi)3 else 1;background=bg(panel,14,Color.rgb(45,52,66));setPadding(dp(14),dp(10),dp(14),dp(10))};fields[k]=e;p.addView(tv(h,12f,muted,true));p.addView(e);return e}
    ...
    private fun showHome(){
        ...
        listOf("Permissão de Trabalho","APR","DDS","EPI","Inspeção","Ocorrência","Colaboradores","Dashboard").forEach{m->
            val c=card(); c.addView(tv(m,18f,if(m=="Permissão de Trabalho")amber else textColor,true)); c.addView(tv(if(m=="Permissão de Trabalho")"PT com riscos, controles, checklist, trabalhadores, fotos e PDF" else "Módulo preparado para expansão",13f,muted,false)); c.addView(btn("Abrir",m=="Permissão de Trabalho"){if(m=="Permissão de Trabalho")showPT() else placeholder(m)}); addCard(c)
        }
    }
    ...
    private fun showPT(){
        clear(); fields.clear(); selectedActs.clear(); risks.clear(); checkStatus.clear(); controlStatus.clear(); workers.clear(); photos.clear()
        content.addView(tv("Permissão de Trabalho",27f,textColor,true)); content.addView(tv("PT nativa com riscos e medidas inteligentes",14f,muted,false))
        val st=card(); statusView=tv("PT BLOQUEADA",17f,red,true); summaryView=tv("",13f,muted,false); st.addView(statusView); st.addView(summaryView); addCard(st)
        val g=card(); section(g,"1. Informações gerais"); listOf("empresa" to "Empresa / Planta","area" to "Área / Setor","local" to "Local da atividade","responsavel" to "Responsável / Emissor","inicio" to "Data/Hora início","fim" to "Data/Hora fim / validade","executantes" to "Equipe / executantes").forEach{input(g,it.first,it.second)}; input(g,"descricao","Descrição detalhada da atividade",true); input(g,"ferramentas","Ferramentas / equipamentos",true); input(g,"produtos","Substâncias / produtos"); addCard(g)
        val a=card(); section(a,"2. Atividades críticas"); riskBank.keys.forEach{act->a.addView(CheckBox(this).apply{text=act;textSize=14f;setTextColor(textColor);buttonTintList=android.content.res.ColorStateList.valueOf(amber);setOnCheckedChangeListener{_,ok->if(ok)selectedActs.add(act) else selectedActs.remove(act);rebuildRisks()}})}; val manual=input(a,"manual","Atividade manual / complemento",true); a.addView(btn("Analisar atividade manual",false){analyzeManual(manual.text.toString());rebuildRisks()}); addCard(a)
        val ch=card(); section(ch,"3. Checklist geral — Sim / Não / N/A"); prereqs().forEachIndexed{i,item->ch.addView(statusRow(item){v->checkStatus[i]=v;refreshStatus()})}; addCard(ch)
        val rc=card(); section(rc,"4. Riscos e medidas de controle"); controlsBox=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL}; rc.addView(controlsBox); addCard(rc)
        val em=card(); section(em,"5. Emergência"); input(em,"ponto","Ponto de encontro"); input(em,"telefone","Ramal / telefone de emergência"); input(em,"emergencia","Procedimento de emergência",true); addCard(em)
        val wf=card(); section(wf,"6. Trabalhadores e fotos"); val wi=input(wf,"worker","Nome do trabalhador + função"); wf.addView(btn("Adicionar trabalhador",false){val v=wi.text.toString().trim();if(v.isNotEmpty()){workers.add(v);wi.setText("");updateLists();refreshStatus()}}); workersView=tv("Nenhum trabalhador adicionado.",13f,muted,false); wf.addView(workersView); wf.addView(btn("Adicionar foto",false){openPhotoChooser()}); photosView=tv("Fotos anexadas: 0",13f,muted,false); wf.addView(photosView); addCard(wf)
        val f=card(); section(f,"7. Finalizar"); f.addView(btn("Gerar e compartilhar PDF",true){sharePtPdf()}); f.addView(btn("Voltar ao menu",false){showHome()}); addCard(f)
        rebuildRisks(); updateLists(); refreshStatus()
    }

    private fun statusRow(label:String,on:(String)->Unit)=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;background=bg(panel,14,Color.rgb(45,52,66));setPadding(dp(10),dp(8),dp(10),dp(8));addView(tv(label,13f,textColor,false));val r=RadioGroup(context).apply{orientation=RadioGroup.HORIZONTAL};listOf("sim" to "Sim","nao" to "Não","na" to "N/A").forEach{(v,n)->r.addView(RadioButton(context).apply{text=n;setTextColor(textColor);buttonTintList=android.content.res.ColorStateList.valueOf(amber);setOnClickListener{on(v)}})};addView(r)}
    private fun analyzeManual(x:String){val t=x.lowercase(); if("mont" in t||"tub" in t||"caixa" in t)risks.addAll(listOf("Prensagem","Corte/perfuração","Queda de objetos","Ergonômicos")); if("altura" in t||"andaime" in t||"escada" in t)risks.addAll(listOf("Queda de pessoas","Queda de objetos")); if("sold" in t||"quente" in t||"corte" in t)risks.addAll(listOf("Incêndio/explosão","Queimadura","Projeção de partículas")); if("escav" in t||"vala" in t)risks.addAll(listOf("Desmoronamento","Interferência subterrânea","Atropelamento")); toast("Atividade manual analisada")}
    private fun rebuildRisks(){val manual=risks.toSet(); risks.clear(); selectedActs.forEach{risks.addAll(riskBank[it]?: emptyList())}; risks.addAll(manual); renderControls(); refreshStatus()}
    private fun renderControls(){val box=controlsBox?:return; box.removeAllViews(); if(risks.isEmpty()){box.addView(tv("Selecione uma atividade ou analise a atividade manual para gerar riscos e medidas.",13f,muted,false));return}; risks.forEach{r->box.addView(tv("Risco: $r",15f,amber,true)); (controlBank[r]?:listOf("Medida de controle a definir")).forEach{c->box.addView(statusRow(c){v->controlStatus["$r|$c"]=v;refreshStatus()})}}}
    private fun refreshStatus(){val pc=prereqs().indices.count{checkStatus[it].isNullOrBlank()};val nc=checkStatus.values.count{it=="nao"};val pend=risks.sumOf{r->(controlBank[r]?:listOf("Medida de controle a definir")).count{c->controlStatus["$r|$c"].isNullOrBlank()}};val blocked=pc>0||nc>0||pend>0||workers.isEmpty();statusView?.text=if(blocked)"PT BLOQUEADA" else "PT LIBERADA";statusView?.setTextColor(if(blocked)red else green);summaryView?.text="Atividades: ${selectedActs.size} • Riscos: ${risks.size} • Checklist pendente: $pc • Controles pendentes: $pend • Trabalhadores: ${workers.size} • Fotos: ${photos.size}"}
